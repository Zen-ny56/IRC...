...
void Server::inviteCommand(int fd, std::string const &message) {
    ...
    if (message.rfind("INVITE ") == 0) {
        ...
        if (targetClientIt != clients.end() && targetClientIt->getChannelName() == channelName) {
        ...
        std::string rpl_inviting = "341 " + client.getNickname() + " " + nickName + " " + channelName + "\r\n";
        send(fd, rpl_inviting.c_str(), rpl_inviting.size(), 0);

        // Send INVITE message to the target user
        std::string inviteMessage = "INVITE " + nickName + " " + channelName + "\r\n";
        targetClientIt = getClientUsingNickname(nickName);
        if (targetClientIt != clients.end()) {
            send(targetClientIt->getFd(), inviteMessage.c_str(), inviteMessage.size(), 0);
        }
    }
}
